/*  =========================================================================
    alert-list_selftest.c - run selftests
...
test_item_t all_tests [] = {
    DECLARE_TEST(bios_alerts_list_server_test),
    {0, 0} // Null terminator
};
...
{
    printf ("Running alert-list selftests...\n");
    test_item_t *item;
    for (item = all_tests; item->test; item++)
        item->test (verbose);
...
main (int argc, char **argv)
{
    ...
    if (test) {
        printf ("Running alert-list selftest '%s'...\n", test->testname);
        test->test (verbose);
    }
    else
        test_runall (verbose);
